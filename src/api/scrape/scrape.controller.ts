...
export class ScrapeController {
  async startScrape(req: Request, res: Response) {
    try {
      ...
      console.log(
        "n8n webhook not configured or unavailable, returning mock response"
      );
      return res.status(200).json({
        status: "success",
  ...
  async processScrapeRequest(req: Request, res: Response) {
    try {
      const { url, scrapeTarget, contentType } = req.body as ScrapeRequest;
      ...
      if (contentType === "html" && !scrapeTarget) {
        return res.status(400).json({
          status: "error",
      ...
      if (error.message && error.message.includes("404")) {
        return res.status(404).json({
          ...
          error: error.message,
          suggestion:
            "Please verify the n8n webhook URL in your environment configuration",
        });
      }
